...
class YOLOv10Node(Node):
    def __init__(self):
        super().__init__('yolov10_detector')
        
        # 参数声明
        self.declare_parameters(
            namespace='',
            parameters=[
                ('model_path', '/home/root/herdeye_car_bayes-e_640x640_nv12_modified.bin'),
                ('camera_id', 0),
                ('show_fps', False),
                ('iou_thres', 0.45),
                ('conf_thres', 0.25),
        ...
        self.detection_pub = self.create_publisher(Detection2DArray, 'sky_detections', 10)
        self.image_pub = self.create_publisher(Image, 'sky_detected_image', 10) if self.get_parameter('publish_image').value else None
        
        # 初始化检测器
        self.detector = YOLOv10_Detect(
        ...
        # 初始化摄像头
        self.cap = cv2.VideoCapture("/home/root/output_1080x720.mp4")
        ...
        self.timer = self.create_timer(1.0 / frame_rate, self.process_frame)
    ...
    def process_frame(self):
        try:
            ...
            ids, scores, bboxes = self.detector.postProcess(outputs)

            # 构建ROS消息
            detection_array = self.create_detection_array(ids, scores, bboxes)
            self.detection_pub.publish(detection_array)

            # 发布可视化图像
            if self.image_pub:
                self.publish_visualization(frame, ids, scores, bboxes, detection_array.header)
    ...
    def create_detection_array(self, ids, scores, bboxes):
        ...
        for class_id, score, bbox in zip(ids, scores, bboxes):
            ...
            # 正确设置BoundingBox
            detection.bbox.center.position.x = (bbox[0] + bbox[2]) / 2
            detection.bbox.center.position.y = (bbox[1] + bbox[3]) / 2
            ...
            hypothesis.hypothesis = ObjectHypothesis(
                class_id=str(coco_names[class_id]),
                score=float(score))
    ...
    def publish_visualization(self, frame, ids, scores, bboxes, header):
        for class_id, score, bbox in zip(ids, scores, bboxes):
