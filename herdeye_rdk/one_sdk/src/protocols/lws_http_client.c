	...
	case LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ:
...
#endif
		lwsl_debug("LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ（user = %p): read len = %zu, in = %p\n",user, len, in);
		http_ctx = (http_request_context_t *)user;
		if (http_ctx == NULL) {
			lwsl_warn("http_ctx is NULL, report issue to developer\n");
			return 0;
		...
		if (http_ctx->client_data->is_sse) {
			...
			if (len + sse->used_len >= sizeof(sse->buffer)) {
					// 如果缓冲区已满，丢弃数据（避免溢出）
					fprintf(stderr, "SSE buffer overflow; truncating data.\n");
					sse->used_len = 0;
					// 处理溢出的逻辑
					// if (sse->buffer[0]) {
						// free(sse->buffer);
			...
			while ((chunk_end = onesdk_memmem(chunk_start, sse->used_len - (chunk_start - sse->buffer), "\n\n", 2))) {
				...
				// 完整事件块准备解析
				lwsl_debug("SSE chunk_len: %zu, bytes : %s\n", chunk_len, chunk_start);
